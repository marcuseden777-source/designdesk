...
function Bone({ className }: { className?: string }) {
  return <View className={`bg-off-white/10 rounded-lg animate-pulse ${className ?? ""}`} />;
}
  ...
  return (
    <SafeAreaView className="flex-1 bg-ink" edges={["top"]}>
      ...
      <View className="flex-row px-5 mt-6 gap-4">
        <View className="flex-1 bg-off-white/[0.06] border border-off-white/12 rounded-2xl p-4">
